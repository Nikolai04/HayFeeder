...
uint8_t a_AdvData[23] =
{
  ...
  10, AD_TYPE_COMPLETE_LOCAL_NAME, 'H', 'a', 'y', 'F', 'e', 'e', 'd', 'e', 'r',  /* Complete name */
...
void APP_BLE_Init(void)
{
  ...
  {
    printf("BLE stack init failed: 0x%02x\r\n", status);
  ...
  printf("BLE advertising requested as HayFeeder\r\n");
...
static void Adv_Request(APP_BLE_ConnStatus_t NewStatus)
{
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    printf("BLE discoverable failed: 0x%02x\r\n", ret);
    APP_DBG_MSG("==>> aci_gap_set_discoverable - fail, result: 0x%x \n", ret);
  }
  else
  {
    printf("BLE discoverable OK\r\n");
...
/* USER CODE END Adv_Request_1*/
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
      printf("BLE advertising data failed: 0x%02x\r\n", ret);
      APP_DBG_MSG("==>> Start Fast Advertising Failed , result: %d \n\r", ret);
  }
  else
  {
      printf("BLE advertising OK: HayFeeder\r\n");
